...
import { l as _sfc_main$a, _ as __nuxt_component_2, b as useRouter } from './server.mjs';
import { g as getSliceComponentProps } from './getSliceComponentProps-DZY85aVg.mjs';
import { defineComponent, computed, mergeProps, withCtx, createVNode, unref, createTextVNode, readonly, ref, reactive, nextTick, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrInterpolate } from 'vue/server-renderer';
import { g as gsapWithCSS } from './index-9D9qVuZg.mjs';
...
const useScrollTrigger = () => {
  ...
  const initGlobalScrollTrigger = () => {
    ...
    (void 0).addEventListener("resize", globalState.resizeHandler);
  ...
  const createContext = (id, setupFunction, options = {}) => {
    cleanupContext(id);
    nextTick(() => {
      const context = gsapWithCSS.context(setupFunction, options.scope || void 0);
  ...
  const batchCreate = (animations) => {
    return animations.map(
      ({ id, config, animationFunction, options }) => createScrollTrigger(id, config, animationFunction, options)
    );
  };
  const cleanup = () => {
    cleanupAllContexts();
  ...
  const destroy = () => {
    ...
    if (globalState.resizeHandler) {
      (void 0).removeEventListener("resize", globalState.resizeHandler);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<footer${ssrRenderAttrs(mergeProps({ class: "footer relative h-screen bg-black" }, _attrs))}><div class="absolute inset-0 z-0 w-full h-full overflow-hidden">`);
      if (_ctx.slice.primary.background_image?.url) {
        _push(ssrRenderComponent(_component_NuxtImg, {
          id: "footer-bg-img",
          format: "avif",
          src: _ctx.slice.primary.background_image.url,
          height: "120%",
          class: "w-full h-full object-cover"
        }, {
          placeholder: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="w-full h-full bg-black"${_scopeId}><h2${_scopeId}>Loading...</h2></div>`);
            } else {
              return [
                createVNode("div", { class: "w-full h-full bg-black" }, [
                  createVNode("h2", null, "Loading...")
                ])
              ];
            }
          }),
      ...
      _push(`</div><div class="relative h-full w-full flex flex-col justify-between px-12 pt-24"><div class="grid grid-cols-10 z-50 mx-auto w-full text-s text-white"><div class="relative flex flex-col gap-4"><!--[-->`);
      ssrRenderList(_ctx.slice.primary.external_links, (item, index) => {
        _push(ssrRenderComponent(_component_PrismicLink, {
          field: item.external_link
        }, null, _parent));
      });
      _push(`<!--]--></div><div class="relative flex flex-col gap-4 col-start-5"><p>© Gauken ${ssrInterpolate(unref(currentYear))}</p>`);
      ...
      _push(`</div><div class="relative flex flex-col gap-4 col-start-7 col-end-10"><p class="text-5xl">Exhibition Yin Yang at Gauken</p></div></div><div class="relative z-50 mx-auto w-full text-xs text-white"><p class="text-[200px] font-medium mb-32 tracking-tight">Gauken</p><div class="w-[500px] h-[200px] bg-white"></div></div></div></footer>`);
    };
  }
});
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("slices/Footer/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=index-C9Qmvdyp.mjs.map
