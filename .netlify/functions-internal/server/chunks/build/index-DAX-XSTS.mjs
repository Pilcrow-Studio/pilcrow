import { a as usePrismic, e as useAsyncData, g as useNuxtApp, u as useHead, _ as __nuxt_component_2, f as _sfc_main$4 } from './server.mjs';
import { _ as __nuxt_component_0 } from './NuxtImg-DXIWJVG8.mjs';
import { defineComponent, withAsyncContext, unref, withCtx, createBlock, createCommentVNode, openBlock, createVNode, toDisplayString, h, ref, useAttrs, getCurrentInstance, Comment, mergeProps, cloneVNode, computed, inject, provide, watch, useSSRContext } from 'vue';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: art_pieces } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "art_pieces",
    ...
    if (ssrContext && ssrContext.res) {
      ...
      if (art_pieces.value?.length) {
        const artPieceTags = art_pieces.value.map((piece) => `art-piece-${piece.id}`).join(",");
        ssrContext.res.setHeader(
          "Netlify-Cache-Tag",
          `homepage,${artPieceTags}`
        );
      } else {
        ssrContext.res.setHeader("Netlify-Cache-Tag", "homepage");
      }
    }
    useHead({
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="min-h-screen"><section><div class="w-full flex flex-row gap-4"><div class="grid grid-cols-3 w-full gap-4"><!--[-->`);
      ssrRenderList(unref(art_pieces), (art_piece) => {
        _push(`<div class="mb-8 max-w-[500px]">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: `/work/${art_piece.uid}`
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (art_piece?.data.artwork?.url) {
                _push2(`<div id="art-piece-image" class="art-image mb-4 shadow-md"${_scopeId}>`);
              ...
              if (art_piece?.data.title) {
                _push2(`<p class="text-md font-serif"${_scopeId}>${ssrInterpolate(art_piece.data.title)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              if (art_piece?.data.price) {
                _push2(`<p class="font-mono uppercase text-sm mt-2"${_scopeId}>${ssrInterpolate(unref(formatCurrency)(art_piece.data.price))}</p>`);
              } else {
                _push2(`<!---->`);
              }
              if (art_piece?.data.size) {
                _push2(`<p class="text-sm mt-2"${_scopeId}>${ssrInterpolate(art_piece.data.size)}</p>`);
              } else {
                _push2(`<!---->`);
              }
            } else {
              return [
                art_piece?.data.artwork?.url ? (openBlock(), createBlock("div", {
                  key: 0,
                  id: "art-piece-image",
                ...
                art_piece?.data.title ? (openBlock(), createBlock("p", {
                  key: 1,
                  class: "text-md font-serif"
                }, toDisplayString(art_piece.data.title), 1)) : createCommentVNode("", true),
                art_piece?.data.price ? (openBlock(), createBlock("p", {
                  key: 2,
                  class: "font-mono uppercase text-sm mt-2"
                }, toDisplayString(unref(formatCurrency)(art_piece.data.price)), 1)) : createCommentVNode("", true),
                art_piece?.data.size ? (openBlock(), createBlock("p", {
                  key: 3,
                  class: "text-sm mt-2"
                }, toDisplayString(art_piece.data.size), 1)) : createCommentVNode("", true)
              ];
            }
          }),
          _: 2
      ...
      _push(`<!--]--></div></div></section><section><div class="container">`);
      _push(ssrRenderComponent(unref(motion).div, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<h1 class="text-4xl font-bold"${_scopeId}>Home</h1>`);
          } else {
            return [
              createVNode("h1", { class: "text-4xl font-bold" }, "Home")
            ];
          }
        }),
        _: 1
...
//# sourceMappingURL=index-DAX-XSTS.mjs.map
